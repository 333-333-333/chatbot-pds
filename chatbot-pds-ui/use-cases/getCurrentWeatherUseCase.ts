...
export async function getCurrentWeather(): Promise<LocalizedWeather | null> {
    const coordinates = await locationRepository.getLocationCoordinates();
    const location = await locationRepository.getLocation();

    if (!coordinates || !location) {
        throw new Error("User current location not found, or no permission was given to request it");
    }

    const weather = await weatherRepository.getCurrentWeather(coordinates);

    if (!weather) {
        throw new Error(`Current weather could not be fetched for location ${location}`);
    }
